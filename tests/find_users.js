...
Tinytest.addAsync("users - find hooks should be capable of being used on wrapped Meteor.users collection", function (test, next) {
  ...
  var Meteor_users_find = Meteor.users.find;

  Meteor.users.find = function (selector, options) {
    selector = selector || {};
    options = options || {};
    return Meteor_users_find.call(this, selector, _.extend({ transform: Meteor.users.__transform }, options));
  };
  ...
  InsecureLogin.ready(function () {
    ...
    Meteor.users.find = Meteor_users_find;
