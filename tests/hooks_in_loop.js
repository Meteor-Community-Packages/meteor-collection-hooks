...
if (Meteor.isClient) {
  ...
  Tinytest.addAsync("issue #67 - hooks should get called when mutation method called in a tight loop", function (test, next) {
    ...
    InsecureLogin.ready(function () {
      Meteor.call("test_hooks_in_loop_reset_collection", function (err, result) {
        function start(id) {
          for (var i = 0; i < times; i++) {
            collection.update({_id: id}, {$set: {times: times}}, function (err) {
              c2++;
              check();
            });
          }
        ...
        collection.insert({times: 0, client_counter: 0, server_counter: 0}, function (err, id) {
          start(id);
        });
      });
    });
