...
if (Meteor.isServer) {
  describe('update - server side', function () {
    it('collection1 document should have extra property added to it before it is updated', async function () {
      const tmp = {}
      ...
      async function start () {
        ...
        expect(await collection1.find({ start_value: true, update_value: true, before_update_value: true }).countAsync()).toBe(2)
...
if (Meteor.isClient) {
  ...
  describe('update - client side', function () {
    it('collection2 document should have client-added and server-added extra properties added to it before it is updated', async function () {
      ...
      async function start (err, id) {
        ...
        collection2.after.update(function (userId, doc, fieldNames, modifier) {
          ...
          expect(Object.prototype.hasOwnProperty.call(this.previous, 'update_value')).toBe(false)
        ...
        // W20240224-16:43:38.769(1)? (STDERR)     at Object.ret.<computed> (packages/mongo/remote_collection_driver.js:52:15)
        ...
        // W20240224-16:43:38.770(1)? (STDERR)     at AsyncLocalStorage.run (node:async_hooks:346:14)
        await collection2.updateAsync({ _id: id }, { $set: { update_value: true } })

        // TODO(v3): this is required for Meteor v2 to work
        await new Promise(resolve => setTimeout(resolve, 100))
        expect(collection2.find({ start_value: true, client_value: true, server_value: true }).count()).toBe(1)
        expect(afterUpdateCalled).toBe(true)
      }

      await Meteor.callAsync('test_update_reset_collection2')
